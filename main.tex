...
Adding noise locally would require a large amount of noise and hence large utility loss to achieve privacy, while globally adding noise after the training step would require some party to have access to the non-noised aggregate. Our protocol adds noise after aggregating intermediate results, but keeps the aggregate secret (\emph{server} differential privacy~\cite{dprio}). To this end, we employ \emph{secure aggregation}, a technique for multiple participants to compute a function on their confidential data that reveals only the computation result without disclosing any onther information about the inputs to anyone.
