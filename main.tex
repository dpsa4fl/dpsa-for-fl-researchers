...
information~\cite{7958568}\cite{Boenisch2021WhenTC}.
...
While removing the need for central data collection, the above scheme still requires clients to disclose the model updates they derived from their private data, which contain sensitive information in itself. 
...
% \section{Our protocol (hoisted)}
% \subsection{from DP}
% Our protocol aims to enable secure aggregation with differential privacy for model update aggregation in order to keep each clients' model updates secret as well as protect their data from inference attacks performed on the aggregated global update or the model resulting from the procedure.
