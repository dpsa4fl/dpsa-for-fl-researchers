...
to perform noising on the aggregation servers prior to recombining the aggregate
shares, so even the aggregators never see the non-noised aggregate gradient (this
...
compromising differential privacy.
...
\subsection{Prio architecture}
 ...
 sums the shares it recieved and publishes the resulting \textit{aggregate
...
checking if the norm of the gradient whose share the server recieved is bounded.
The prio protocol provides \textit{secret-shared non-interactive
  proofs}~\cite[Section 4]{prio} to allow the servers to jointly validate this
property without disclosing any clients' submissions. Correct validation
requires all servers to follow the validation protocol honestly, but does not
disclose any information about honest clients even if all but one server
deviate.
...
These aggregation servers do not need to be
...
collector, each aggregation server adds noise to their own aggregate share, in
...
frameworks, using flower as target framework.
...
The Prio is designed to mitigate this sort of attack. Even though the aggregation
...
poisoning attacks by a minority of clients are mitigated. The Prio can only
