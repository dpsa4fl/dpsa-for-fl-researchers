...
We present \texttt{dpsa4fl}, a system which provides differential privacy for
federated gradient descent. Our privacy guarantees cover the training result,
...
\paragraph{Related work.} Our work covers a broad range of topics, and as such,
there are various projects and papers related to a specific subsets.

\textit{Secure aggregation with input validation.} We are specifically
interested in secure aggregation where the input of clients can be verified to
fullfill certain properties. Such protocols can be roughly categorized by
whether they assume a single or multiple aggregation servers. In the former
category there are \textit{Acorn}~\cite{acorn}, \ldots

\textit{Privacy for FL.} Various existing federated learning frameworks provide privacy.


% \textit{Rest.} There is a variety of work on federated learning with secure aggregation and global differential privacy. Systems that provide both differ from ours by the assumed threat model:
% \begin{itemize}
